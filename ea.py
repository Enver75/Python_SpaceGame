...
kalsor = os.path.dirname(__file__)
resimKlasoru = os.path.join(kalsor,"resimler")
sesKlasoru = os.path.join(kalsor,"sesler")
########################################################################################################################

#######################      P A T L A M A    K L A S O R L E R U      #################################################

patlamaKlasoru = os.path.join(kalsor,"patlama")
patlamaResimleri = []
for i in range(1,10):
    patlamaResimleri.append("{}.png".format(i))
########################################################################################################################

#####################        G U C L E N D İ R İ C İ    R E S İ M L E R İ         ######################################
...
pygame.mixer_music.load(os.path.join(sesKlasoru,"ae.mp3"))
pygame.mixer_music.play()
#pygame.mixer_music.set_volume(0.1)
#########################################################################################################################

###########################      M Ü Z İ K   E F E K T L E R          ##################################################
...
font = pygame.font.SysFont("Gravity",50)
...
###############       O Y U N U N   A N A   E K R A N  B A Ş L A N G I C   A Y A R I             #######################
...
while True:
    ...
    fontScore = font.render("Kalan Mermi : {}".format(mermiSayisi),1,(0,255,0))
...
#############     A s t e r o i d   ile   U z a y    G e m i s i n i n i    Ç a r p ı ş m a s ı   ######################

    durum = pygame.sprite.spritecollide(parca1,mermiler,True,collided=pygame.sprite.collide_circle)
########################################################################################################################

#####################     M e r m i l e r   ile   A s t e r o  i d    Ç a r p ı ş m a s ı     ##########################
    ...
    if durum or mermiSayisi == 0:
        if parca1.kalkan <= 0:
            pygame.mixer_music.load(os.path.join(sesKlasoru, "explode.mp3"))
            pygame.mixer_music.play()
            parca1.can -= 1
            parca1.hide()
        ...
        if mermiSayisi == 0:
            if sayacSifirlama:
                ...
                levelYaziFont = pygame.font.SysFont ("Gravity",40)
                yazi = levelYaziFont.render("Level {}".format(level+1),1,(255,255,224))
